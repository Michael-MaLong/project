...
class Contentinfo(models.Model):
    ...
    changeflag = models.CharField(u"变更状态", max_length=2, choices=CHANGE_CHOICES,
                                  blank=True) #0 代表无变更；1 代表新增；2 代表变更；3 代表删除；4 代表子节点有变更
    ...
    def Children(self):
        return Contentinfo.objects.filter(superid=self.absname,checkflag="1",serverid=self.serverid).order_by('ftype', '-changeflag')
