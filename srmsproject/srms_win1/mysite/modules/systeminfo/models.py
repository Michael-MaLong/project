...
class Systeminfo(models.Model):
    ...
    @staticmethod
    def get_param_value(param_name, param_type='1'):
        global SYSVERSION
        try:
            param_value = cache.get("sys_config_%s_%s_%s" % (SYSVERSION, param_type, param_name))
            if param_value:
                return param_value
            systeminfolist = Systeminfo.objects.filter(param_name=param_name, param_type=param_type)
            if systeminfolist:
                systeminfo = systeminfolist[0]
                param_value = systeminfo.param_value
                cache.set("sys_config_%s_%s_%s" % (SYSVERSION, param_type, param_name), param_value)
            else:
                param_value = ""
            return param_value
        except:
    ...
    def get_param(param_name):
        ...
        try:
            ...
            if systeminfo.exists():
                ...
                cache.set("sys_config_%s_%s" % (param_name, SYSVERSION), result)
            return result
        except:
            traceback.print_exc()
            return {param_name: ""}
